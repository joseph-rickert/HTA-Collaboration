---
title: "Be Careful of the Time: Estimating Continuous Time Markov Chains from Incomplete Data"
format: html
---

**Needs to be reworked**
This post presents a likelihood method to develop a 5-state continuous time Markov chain model to incomplete data from a clinical trial that compared multiple treatments for asthma. The analysis complements the a previously published post *A Simple Bayesian Multi-state Survival Model for a Clinical Trial* replicated a textbook example of fitting a discrete time Markov chain to the data. The analysis compares the two models and highlights the importance of carefully the treating time in Markov models based on incomplete data. 


In a previous post, [A Simple Bayesian Multi-state Survival Model for a Clinical Trial](https://rworks.dev/posts/simple-bayesian-model/), I elaborated on a discrete time Markov Chain model that Welton et al. (2010) used to analyze data from two arms of a clinical trial conducted by Kavuru et al. (2000) comparing multiple treatments for asthma management.  In this post, I extend the analysis of the Seretide and Flucticasone comparison by fitting a Continuous Time Markov Chain to the same minimalist data set using a likelihood based approach. Thus, not only do we have a comparision between a Bayesian and frequentist methods, but also between discrete time and continuous time Markov chains. This latter comparison highlights the importance of explicitly accounting for time in modeling real world processes. 


## It's A Matter of Time

The choice between using a discrete time Markov chain (DTMC) or a continuous time Markov chain (CTMC) to model a real world process is mostly about time.
...
As in the previous post, I follow Welton et al. and confine my analysis to comparing the two treatments Seretide and Flucticasone. The Seretide and Flucticasone data are reproduced in in Welton et al. (2010) and in the paper by Briggs et al. (2003). The data for both of these treatments, sometimes described as "incomplete discrete data" are just state tables that comprise the aggregate number of state transition observed over the course of 12 weeks. (The states are defined as follows: `STW`= successfully treated week, `UTW`= unsuccessfully treated week, `Hex`= hospital-managed exacerbation ,`Pex`= primary care-managed exacerbation, and `TF`= treatment failure.) This is probably the most minimal data set that could be used to fit a multi-state Markov model. Moreover, the data is sparse. For Seretide, only two state transitions were observed for `UTW` and `Hex` combined.
...
Yasunari Inamura, Estimating Continuous Time Transition Matrices From Discretely Observed Data, Bank of
...
The integral equations requred to estimate the likelihood function for the $Q$ matrix from incomplete data are: 
