...
model{
  ...
  # Calculating summaries from a decision model
	for (tmt in 1:2){ 
		...
		for (i in 1:4){  
			for (t in 2:13){
				s[tmt,i,t] <- inprod(s[tmt,1:4,t-1], pi[tmt,1:4,i]) 	# Run the model for N cycles. 
																									# s[tmt,i,t] = no. in state i at time t under treatment tmt
				}
			E[tmt,i] <- sum(s[tmt,i,2:13])							# Sum up time spent in state i
			}
		E[tmt,5] <- 12 - sum(E[tmt,1:4])							# Time in TF = 13 minus time in other states.
	}
...
#| code-fold: true
#| code-summary: "Show the code"
system.time(jags_results <- jags(data = data,
